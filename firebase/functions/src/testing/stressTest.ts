...
async function runStressTest(): Promise<void> {
  ...
  const snapshot = await db.collection('teachers_raw').where('batchId', '==', batchId).get();
  ...
  const commLogBatches: admin.firestore.WriteBatch[] = [];
  ...
  console.log(
    `  Throughput: ${(messagesCreated / (msgDuration / 1000)).toFixed(0)} messages/sec\n`,
  );
  ...
  console.log(
    `Overall throughput: ${(teachers.length / (totalDuration / 1000)).toFixed(0)} teachers/sec`,
  );
  console.log('===============\n');
}
